...
(use-package perspective
  ...
  ;; 快捷键绑定
  (global-set-key (kbd "C-x k") 'persp-kill)  ;; 删除当前perspective
  (global-set-key (kbd "C-x C-b") 'persp-list-buffers)  ;; 列出perspective中的缓冲区
  (global-set-key (kbd "C-x C-p") 'persp-switch)  ;; 切换perspective
  (global-set-key (kbd "C-x p n") 'persp-next)  ;; 切换到下一个perspective
  (global-set-key (kbd "C-x p p") 'persp-prev)  ;; 切换到上一个perspective
  (global-set-key (kbd "C-x p c") 'persp-copy)  ;; 复制当前perspective
